...
def hough_line(edges, min_line_length=200, max_line_gap=10):
    lines = cv2.HoughLines(edges, 1, np.pi / 180, 200, min_line_length, max_line_gap)
...
def write_crop_images(img, points, img_count=0, folder_path='./Data/predict/'):
    num_list = []
    ...
    for row in num_list:
        for s in row:
            ...
            img_count += 1
            cv2.imwrite('./Data/predict/data_image' + str(img_count) + '.jpeg', cropped)
...
def grab_cell_files(folder_name='./Data/predict/'):  
    return sorted([os.path.join(folder_name, x) for x in os.listdir(folder_name) if x.startswith('data')])
